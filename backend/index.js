...
app.get('*', (req, res) => {
  ...
  // Check if the build directory exists, if not, send a simple message
  if (fs.existsSync(indexPath)) {
    res.sendFile(indexPath);
  } else {
    res.status(404).send(`
      <html>
        <head><title>Property Search App</title></head>
        <body>
          <h1>Property Search App</h1>
          <p>Frontend not built yet. Please run: <code>cd frontend && npm run build</code></p>
          <p>API is available at: <a href="/api/query_properties">/api/query_properties</a></p>
        </body>
      </html>
    `);
  }
